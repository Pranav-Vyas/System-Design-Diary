...
// Note: At here, we are declaraing the handlers beforehand. But what if we want to set the next handler at runtime?
/*
- we can put a setHandler method instead of setting handler at the time of object creation

abstract class OrderHandler {
    protected OrderHandler nextHandler;

    public setNextHandler(OrderHandler nextHandler){
      this.nextHandler = nextHandler;
    }
...
- and at runtime we can set the next handler and use it
